...
def o__in_top_k_loss_single(y_true, y_pred):
    y_true_labels = tf.cast(tf.transpose(tf.where(y_true > 0))[0], tf.int32)
...
def calculate_top_k_new_only(model, A_test, X_test, y_test, batch_size, include_time_dim_in_X):
    y_pred = model.predict(A_test, X_test, batch_size)
    return (-1) * K.eval(
...
def o__in_top_k_loss(y_true, y_pred):
    return K.mean(tf.map_fn(lambda x: o__in_top_k_loss_single(x[0], x[1]), (y_true, y_pred), dtype=tf.float32))
